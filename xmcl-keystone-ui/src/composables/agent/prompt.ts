...
export function renderAgentSessionContext(profile: Pick<AgentPromptProfile, 'role' | 'sessionContext'>) {
  ...
  if (profile.role === 'css-main') return 'Global DHMC custom CSS scope.'
...
function capabilitySection(profile: AgentPromptProfile) {
  ...
  if (names.has('vfs_shell')) {
    lines.push('The `vfs_shell` tool is a virtual DHMC command runner scoped to the selected instance, not an operating-system shell. It accepts exactly one DHMC command and cannot inspect launcher source code, arbitrary host paths, or a repository checkout. Do not use shell utilities, pipes, redirects, `&&`, or `||`, and do not prefix the input with `vfs_shell`.')
...
const launcherIdentity = `You are Discohead, the primary DHMC assistant. Help the user manage the explicitly selected Minecraft instance, resources, Java, local servers, worlds, launch failures, and launcher settings.`
const cssIdentity = `You are the Discohead Custom CSS assistant. Your only responsibility is inspecting the launcher UI and maintaining the global custom CSS document. Do not manage Minecraft instances, resources, accounts, or game launch.`
const modpackChangelogIdentity = `You are the Discohead Modrinth modpack release-notes writer. Your only responsibility is proposing a version number and a changelog for a new modpack version, based on the provided mod/file diff and any draft notes.`
