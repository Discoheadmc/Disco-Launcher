...
export function useAgentToolFactory() {
  ...
  async function createVfsShellTool(context: AgentRunContext, vfs: Awaited<ReturnType<typeof createVfsTools>>, runtimeChange: AgentRuntimeChangeTracker): Promise<AgentTool> {
    ...
    return {
      name: 'vfs_shell',
      label: 'DHMC command',
      description: 'Run one virtual DHMC command. This is not an OS shell and cannot inspect source code, repositories, or arbitrary host files; use `help` to discover supported commands.',
...
export function useCssAgentToolFactory() {
  ...
  const uiTool: AgentTool = {
    name: 'ui',
    label: 'Launcher UI',
    description: 'Inspect the DHMC user interface for CSS selectors and computed styles.',
